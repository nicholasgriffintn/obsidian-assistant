...
import { AssistantSettingsTab } from "./settings";
import { ChatModal } from "./modals/chat";
import { SyncService } from "./services/sync";
import type { AssistantPluginSettings } from "./types";
...
import { createAssistant, type AssistantProvider } from "./services/ai";

export default class AssistantPlugin extends Plugin {
	public settings!: AssistantPluginSettings;
	private assistant!: AssistantProvider;
	...
	private async initializeServices(): Promise<void> {
		const decryptedAssistantApiKey = this.getDecryptedApiKey(
			this.settings.assistantApiKey,
		);

		this.assistant = createAssistant({
			apiKey: decryptedAssistantApiKey,
			baseURL: this.settings.assistantApiUrl,
		});
			...
			this.app.metadataCache.on("deleted", async (file) => {
				try {
					if (file instanceof TFile && file.extension === "md") {
						...
						await this.assistant.deleteEmbedding([vectorId]);

						const syncPath = `.obsidian-assistant/sync/${vectorId}.json`;
